...
export const apiService = {
  ...
  getCheckDetails: async (checkNumber) => {
    // має йти GET запит на ваш бекенд, щоб дістати Sales для конкретного Check
  ...
  createEmployee: async (data) => { if (USE_MOCK) { await delay(300); return { ...data, id_employee: `emp_${Date.now()}` }; } return (await axiosClient.post('/employees', data)).data; },
  ...
  updateProduct: async (id, data) => { if (USE_MOCK) { await delay(300); return data; } return (await axiosClient.put(`/products/${id}`, data)).data; },
  deleteProduct: async (id) => { if (USE_MOCK) { await delay(300); return { success: true }; } return await axiosClient.delete(`/products/${id}`); },
  getProductById: async (id) => { if (USE_MOCK) { await delay(100); return mockDb.mockProducts.find(p => p.id_product === id); } const res = await axiosClient.get(`/products/${id}`); return res.data; },
  getProducts: async () => { if (USE_MOCK) { await delay(200); return [...mockDb.mockProducts]; } const res = await axiosClient.get('/products'); return res.data; },
  ...
  getChecks: async () => {
    if (USE_MOCK) { await delay(200); return [...mockDb.mockChecks]; }
    const res = await axiosClient.get('/checks');
    return res.data;
  },
  getCheckDetails: async (checkNumber) => {
    if (USE_MOCK) {
        await delay(100);
        return [ { name: 'Молоко 2.5%', qty: 2, price: 28.40 }, { name: 'Батон', qty: 1, price: 20.00 } ];
    }
    // Тепер запит йде до нашого нового методу на бекенді
    const res = await axiosClient.get(`/checks/${checkNumber}/items`);
    return res.data; 
  },
};