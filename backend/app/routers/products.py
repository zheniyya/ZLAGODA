...
def create_product(product: ProductBase, current_user: dict = Depends(require_manager), conn = Depends(get_db_connection)):
...
def get_all_products(current_user: dict = Depends(get_current_user), conn = Depends(get_db_connection)):
    with conn.cursor() as cur:
        cur.execute("SELECT * FROM Product ORDER BY product_name")
        return cur.fetchall()

@router.get("/{id_product}", response_model=ProductResponse)
def get_product(id_product: int, current_user: dict = Depends(get_current_user), conn = Depends(get_db_connection)):
    with conn.cursor() as cur:
...
def update_product(id_product: int, product: ProductBase, current_user: dict = Depends(require_manager), conn = Depends(get_db_connection)):
    with conn.cursor() as cur:
        cur.execute("""
...
def delete_product(id_product: int, current_user: dict = Depends(require_manager), conn = Depends(get_db_connection)):
    with conn.cursor() as cur:
